...
UpdateManager::UpdateManager(int version)
{
    ...
    feedURL = "https://github.com/PhilInTheGaps/GM-Companion/releases.atom"; // GitHub Release feed
...
void UpdateManager::checkForUpdates()
{
    ...
    networkManager->get(QNetworkRequest(QUrl(feedURL))); // Get the release feed to check for a new version
}
        ...
        if (reader.name() == "feed")
        {
            while(reader.readNextStartElement())
            {
                if(reader.name() == "entry")
                {
                    ...
                    int versionNumber = versionString.replace(".", "").toInt(); // Remove all the dots to make it a number and easier to compare
                    ...
                    if (versionNumber > newestVersion)
                    {
                        ...
                        // Forgot why I did not simply use versionString, but must have had a reason
