...
void AbstractAccessTest::getDataAsync()
{
    ...
    testFuture(future2, QStringLiteral("File::getDataAsync"), [future2]() {
        QVERIFY2(!future2.isCanceled(), "QFuture is canceled!");
...
void AbstractAccessTest::moveAsync()
{
    // Move existing file
    auto future =
        File::moveAsync(getFilePath(QStringLiteral("test5")), getFilePath(QStringLiteral("test5-moved")), fileAccess);
    ...
    auto future2 = File::moveAsync(getFilePath(QStringLiteral("test5-moved")),
                                   getFilePath(QStringLiteral("moves/test1")), fileAccess);
    ...
    expectWarning();
    auto future4 =
        File::moveAsync(getFilePath(QStringLiteral("test5-moved")), getFilePath(QStringLiteral("test1")), fileAccess);
    testFuture(future4, QStringLiteral("File::moveAsync"), [future4]() {
        QVERIFY2(!future4.isCanceled(), "QFuture is canceled!");
...
void AbstractAccessTest::copyAsync()
{
    ...
    auto future2 = File::copyAsync(getFilePath(QStringLiteral("test7-missing")),
                                   getFilePath(QStringLiteral("test7-copy2")), fileAccess);
    ...
    expectWarning();
    auto future3 =
        File::copyAsync(getFilePath(QStringLiteral("test7-copy")), getFilePath(QStringLiteral("test1")), fileAccess);
...
void AbstractAccessTest::listAsync()
{
...
#ifndef Q_OS_WIN
        QVERIFY2(future.result()->files().contains("file&with\"special characters"),
    ...
    testFuture(future2, QStringLiteral("File::listAsync"), [this, future2]() {
        ...
        {
            QCOMPARE(FileUtils::dirFromPath(file), getFilePath());
        }

        for (const auto &folder : future2.result()->foldersFull())
        {
            QCOMPARE(FileUtils::dirFromPath(folder), getFilePath());
        }
    ...
    testFuture(future3, QStringLiteral("File::listAsync"), [future3]() {
        ...
        QVERIFY2(!future3.result()->folders().contains("test1"),
