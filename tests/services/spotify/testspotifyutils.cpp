...
void TestSpotifyUtils::getUriType()
{
    ...
    qDebug() << (int)SpotifyUtils::getUriType(uri) << (int)type;
...
void TestSpotifyUtils::getIdFromHref_data()
{
    ...
    QTest::newRow("album") << "https://api.spotify.com/v1/albums/6akEvsycLGftJxYudPjmqK/tracks?offset=0&limit=2"
    ...
    QTest::newRow("track") << "https://api.spotify.com/v1/tracks/5TiMM0jscVmWgMwR42RLup"
                           << "5TiMM0jscVmWgMwR42RLup";
    QTest::newRow("artist") << "https://api.spotify.com/v1/artists/2d0hyoQ5ynDBnkvAbJKORj"
                            << "2d0hyoQ5ynDBnkvAbJKORj";
