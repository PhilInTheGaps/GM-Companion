...
auto SpotifyImageLoader::loadPlaylistImageAsync(const QString &id) -> QFuture<QPixmap>
{
    const auto playlistCallback = [id](const SpotifyPlaylist &playlist) {
        ...
        auto future = Services::Spotify::instance()->get(url, false, true);
    ...
    auto future = Spotify::instance()->playlists.getPlaylist(id, true);
...
void SpotifyImageLoader::startRequest(SpotifyUtils::SpotifyType type)
{
    ...
    auto future = Spotify::instance()->get(url, true, true);
...
void SpotifyImageLoader::receivedRequest(RestReply &&reply, SpotifyUtils::SpotifyType type)
{
    ...
    for (const auto &value : entries)
    {
        ...
        auto future = Spotify::instance()->get(request, false, true);
