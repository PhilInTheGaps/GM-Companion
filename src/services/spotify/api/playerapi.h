...
public:
    ...
    [[nodiscard]] auto play(const QString &deviceId, const QJsonObject &body) const -> QFuture<gsl::owner<RestNetworkReply*>>;
    ...
    [[nodiscard]] auto getState() const -> QFuture<QSharedPointer<SpotifyPlaybackState>>;
    [[nodiscard]] auto getState(const QStringList &additionalTypes, const QString &market) const -> QFuture<QSharedPointer<SpotifyPlaybackState>>;

    /// Get the object currently being played on the user's Spotify account
    [[nodiscard]] auto getCurrentlyPlaying() const -> QFuture<QSharedPointer<SpotifyCurrentTrack>>;
    [[nodiscard]] auto getCurrentlyPlaying(const QStringList &additionalTypes, const QString &market) const -> QFuture<QSharedPointer<SpotifyCurrentTrack>>;
    ...
    [[nodiscard]] auto transfer(const QStringList &deviceIds, bool play) const -> QFuture<gsl::owner<RestNetworkReply*>>;

    /// Get information about a user’s available devices
    [[nodiscard]] auto devices() const -> QFuture<QSharedPointer<SpotifyDeviceList>>;
    ...
    [[nodiscard]] auto repeat(SpotifyRepeatMode mode, const QString &deviceId) const -> QFuture<gsl::owner<RestNetworkReply*>>;

    /// Set the repeat mode for the user's playback. Options are repeat-track, repeat-context, and off (specific device)
    [[nodiscard]] auto repeat(const QString &state, const QString &deviceId) const -> QFuture<gsl::owner<RestNetworkReply*>>;
    ...
    [[nodiscard]] auto volume(int volumePercent, const QString &deviceId) const -> QFuture<gsl::owner<RestNetworkReply*>>;
    ...
    /// Get tracks from the current user's recently played tracks
    /// Note: "limit" will be clamped to between 0 and 50
    [[nodiscard]] auto getRecentlyPlayed(const QDateTime &after, const QDateTime &before, int limit) const -> QFuture<gsl::owner<RestNetworkReply*>>;
    ...
    [[nodiscard]] auto addToQueue(const QString &uri, const QString &deviceId) const -> QFuture<gsl::owner<RestNetworkReply*>>;

private:
    explicit PlayerAPI(Spotify *parent);
    const QPointer<Spotify> m_spotify = nullptr;

    static auto makePlayBody(const QString &contextUri, const QStringList &uris, int position, int positionMs) -> QJsonObject;
