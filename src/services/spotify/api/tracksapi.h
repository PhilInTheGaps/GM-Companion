...
public:
    ...
    [[nodiscard]] auto getTracks(const QStringList &ids) -> QFuture<std::vector<QSharedPointer<SpotifyTrack>>>;

private:
    explicit TracksAPI(Spotify *parent);
    const QPointer<Spotify> m_spotify = nullptr;

    [[nodiscard]] auto getTracks(const QStringList &ids, std::vector<QSharedPointer<SpotifyTrack>> &&previous)
