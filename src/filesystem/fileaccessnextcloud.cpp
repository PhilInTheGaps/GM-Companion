    ...
    const std::function<QFuture<FileResult *>(const T1 &, const T2 &)> &func) -> QFuture<FileResult *>
{
    return createDirAsync(dir)
        .then(this,
              [arg1, arg2, func](FileResult *result) {
    ...
    return future
        .then(this,
              [this, future, path, data](QNetworkReply *reply) {
                  return QtFuture::connect(reply, &QNetworkReply::finished).then(this, [this, path, data, reply]() {
                      if (replyHasError(reply))
                      {
                          ...
                          return deleteReplyAndReturn(QtFuture::makeReadyFuture(new FileResult(errorMessage, this)),
                                                      reply);
                      }
                      ...
                      return deleteReplyAndReturn(QtFuture::makeReadyFuture(new FileResult(true, this)), reply);
                  });
              })
        .unwrap();
}
    ...
    return future
        .then(
            ...
            [this, future, oldPath, newPath](QNetworkReply *reply) {
                return QtFuture::connect(reply, &QNetworkReply::finished).then(this, [this, oldPath, newPath, reply]() {
                    if (replyHasError(reply))
                    {
...
auto FileAccessNextcloud::deleteAsync(const QString &path) -> QFuture<FileResult *>
{
    ...
    return future
        .then(this,
              [this, future, path](QNetworkReply *reply) {
                  return QtFuture::connect(reply, &QNetworkReply::finished).then(this, [this, path, reply]() {
                      if (replyHasError(reply))
                      {
                          const auto errorMessage =
    ...
    return future
        .then(this,
              [this, future, path, copy](QNetworkReply *reply) {
                  return QtFuture::connect(reply, &QNetworkReply::finished).then(this, [this, path, copy, reply]() {
                      if (replyHasError(reply))
                      {
                          const auto isDirMissing = reply->error() == QNetworkReply::ContentConflictError;
                          if (isDirMissing)
        ...
        .then(this,
              [this, future, path, files, folders](QNetworkReply *reply) {
                  return QtFuture::connect(reply, &QNetworkReply::finished)
                      .then(this, [this, path, files, folders, reply]() {
                          if (replyHasError(reply))
                          {
                              const auto errorMessage =
