    ...
    return state.fa->getFileIdAsync(state.oldPath)
        .then(state.fa->context(),
              [state = std::move(state)](const QString &fileId) mutable {
                  if (fileId.isEmpty())
                      return QtFuture::makeReadyFuture(FileResult(u"Could not find file/folder ID"_s));

                  state.fileId = fileId;
                  return onFileIdReceived(std::move(state));
              })
        .unwrap()
        .onCanceled(state.fa->context(), []() { return FileResult(u"Could not find file/folder ID"_s); });
}
...
auto MoveImpl::onFileIdReceived(State &&state) -> QFuture<FileResult>
{
    return state.fa->getParentIdAsync(state.oldPath)
        .then(state.fa->context(),
              [state = std::move(state)](const std::pair<QString, QString> &oldParent) mutable {
                  state.oldParentId = oldParent.second;
                  return onParentIdReceived(std::move(state));
              })
        .unwrap()
        .onCanceled(state.fa->context(), []() { return FileResult(u"Could not find ID of old parent folder"_s); });
}
...
auto MoveImpl::onParentIdReceived(State &&state) -> QFuture<FileResult>
{
    return state.fa->getParentIdAsync(state.newPath)
        .then(state.fa->context(),
              [state = std::move(state)](const std::pair<QString, QString> &newParent) mutable {
                  state.newParentId = newParent.second;
                  return onNewParentIdReceived(std::move(state));
              })
        .onCanceled(state.fa->context(),
                    [state = std::move(state)]() mutable { return createNewParentFolderAndTryAgain(std::move(state)); })
        .unwrap();
}
...
auto MoveImpl::onNewParentIdReceived(State &&state) -> QFuture<FileResult>
{
    ...
    return state.fa->m_gd.customRequest(request, "PATCH", metadata)
        ...
        .onCanceled(state.fa->context(), []() { return FileResult(u"Could not send PATCH request"_s); });
}
...
auto MoveImpl::createNewParentFolderAndTryAgain(State &&state) -> QFuture<FileResult>
{
    ...
    return state.fa->createDirAsync(parentPath)
        ...
        .unwrap()
        .onCanceled(state.fa->context(), []() { return FileResult(u"Could not create new parent folder"_s); });
}
