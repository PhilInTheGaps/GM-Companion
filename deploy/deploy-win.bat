...
mkdir release\deploy || EXIT /B 1

copy /y src\gm-companion.exe release\deploy

windeployqt.exe -xml --release --qmldir %QT_PATH%\qml release\deploy\gm-companion.exe || EXIT /B 1

copy /y "%OPENSSL%\bin\*eay32.dll" release\deploy
copy /y "%OPENSSL%\bin\lib*.dll" release\deploy

echo "Downloading librespot ..."
curl -L -o release\deploy\librespot.exe https://github.com/PhilInTheGaps/librespot-bin/releases/download/v0.1.1/librespot-x86_64.exe || EXIT /B 1
